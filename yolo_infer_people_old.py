...
def main():
    ...
    yes_path = "/media/vs/Extreme SSD/QiMin/SSH/21019/yes_people"
    ...
    for root, dirs, files in os.walk(r"/media/vs/Extreme SSD/QiMin/SSH/21019/0606"):  # 这里就填文件夹目录就可以了
        for file in files:
            # 获取文件路径
            if '.jpg' in file:
                ...
                for label, confidence, bbox in detections:
                    if 'person' in label:
                    # if 'wear helmet' or 'no helmet' in label:
                        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

...
def parser():
    ...
    parser.add_argument("--weights",
                        default="/media/vs/Data/darknet_train_result/darknet/Helmet/helmet_new/Big/weights/HelmetBig_380000_20210730.weights",
                        help="yolo weights path")
    parser.add_argument("--dont_show", action='store_true',
                        help="windown inference display. For headless systems")
    parser.add_argument("--ext_output", action='store_true',
    ...
    parser.add_argument("--config_file",
                        default="/media/vs/Data/darknet_train_result/darknet/Helmet/helmet_new/Big/model/20210730/HelmetBig.cfg",
                        help="path to config file")
...
def main():
    ...
    yes_path = "/media/vs/Data/darknet_train_result/darknet/Helmet/helmet_new/PR/all_test/detection2"
    if not os.path.exists(yes_path):
        os.makedirs(yes_path)
    # no_path = "/media/wst/Data/darknet训练结果/all_train/helmet/no_truck"
    i = 0
    # j = 0

    for root, dirs, files in os.walk(r"/media/vs/Data/darknet_train_result/darknet/Helmet/helmet_new/PR/all_test/images"):  # 这里就填文件夹目录就可以了
        for file in files:
            # 获取文件路径
            if '.jpg' in file:
                ...
                list = ['truck_head','close','open','Incomplete close','others']
                for label, confidence, bbox in detections:
                    # for _ in list:

                    # if 'person' in label:
                    # if label == 'person':
                    if label == 'wear helmet' or label == 'no helmet':
                        x, y, w, h = bbox
                        size1 = (w * h)  # 预测框
                        size2 = (x_ * y_)
                        k = size1 / size2
                    ...
                    #     # os.makedirs(os.path.join(yes_path, root))
                    ...
                    # else:
                    #     dst2 = os.path.join(no_path, '0' + file[:-4] + str(j) + '.jpg')
                    #     shutil.copy(path, dst2)
                    #     j += 1
                    #     print('j = ', j)
    # print('有人图片 %s 张' % i)
    # print('有truck图片 %s 张' % i)
    print('识别图片 %s 张' % i)
